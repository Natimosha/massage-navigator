...
function getPersonalizedSteps(state) {
    ...
    if (state.workMode === 'salon-only' && state.energyVector === 'exit') {
        ...
        steps.push({
            ...
            pdfBlocks: ['place-comparison', 'place-checklist']
        });
    ...
    // ═══════════════════════════════════════════════════════════════════
        ...
        steps.push({
            ...
            title: 'Усилить свои каналы привлечения',
            detail: 'Соцсети, Авито, Яндекс.Карты — отдельно от салона.',
            metric: 'Цель: 2-3 канала по 3-5 клиентов/мес',
            pdfBlocks: ['channel-avito', 'channel-yandex-maps', 'channel-social']
        });
    ...
    // ═══════════════════════════════════════════════════════════════════
        ...
        steps.push({
            ...
            title: 'Усилить свои каналы привлечения',
            detail: 'Соцсети, Авито, Яндекс.Карты — своя аудитория.',
            metric: 'Цель: 2-3 канала по 3-5 клиентов/мес',
            pdfBlocks: ['channel-avito', 'channel-yandex-maps', 'channel-social']
        });
    ...
    // ═══════════════════════════════════════════════════════════════════
        ...
        if (state.sources && state.sources.length <= 2) {
            steps.push({
                ...
                title: 'Добавить 2 источника клиентов',
                detail: 'Сейчас ' + state.sources.length + '. Каждый канал — страховка.',
...
или на WhatsApp: [номер]</div>
    ...
    <div class="page">
        ...
        <div class="numbered-list">
            <li>Попросите друзей подписаться и сделать репост</li>
            <li>Добавьте ссылку в WhatsApp-статус и подпись</li>
            <li>Разместите QR-код в кабинете</li>
            <li>После сеанса: «Подпишитесь, там полезные советы»</li>
            <li>Комментируйте посты в местных сообществах</li>
        </div>
...
// С АВТОМАТИЧЕСКИМ РАЗБИЕНИЕМ ДЛИННЫХ СТРАНИЦ
// ═══════════════════════════════════════════════════════════════════════════
...
async function generateAndDownloadPdf(state, filename) {
    ...
    // A4 размеры в пикселях при scale=2: 210mm * 2 * 3.78 ≈ 1587px ширина
    // Высота A4: 297mm * 2 * 3.78 ≈ 2245px
    const A4_HEIGHT_PX = 2245; // при scale=2
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        if (canvasHeight <= A4_HEIGHT_PX) {
            ...
            pdf.addImage(imgData, 'JPEG', 0, 0, A4_WIDTH_MM, imgHeight);
        } else {
            // Контент больше одной страницы - разбиваем
            const numPages = Math.ceil(canvasHeight / A4_HEIGHT_PX);
            ...
            for (let j = 0; j < numPages; j++) {
                ...
                // Создаём временный canvas для части изображения
                const partCanvas = document.createElement('canvas');
                partCanvas.width = canvasWidth;
                
                // Высота этой части
                const startY = j * A4_HEIGHT_PX;
                const partHeight = Math.min(A4_HEIGHT_PX, canvasHeight - startY);
                partCanvas.height = partHeight;
                
                const partCtx = partCanvas.getContext('2d');
                
                // Копируем часть оригинального canvas
                partCtx.drawImage(
                    canvas, 
                    0, startY,           // source x, y
                    canvasWidth, partHeight,  // source width, height
                    0, 0,                // dest x, y
                    canvasWidth, partHeight   // dest width, height
                );
                
                const partImgData = partCanvas.toDataURL('image/jpeg', 0.95);
                const partHeightMM = (partHeight * A4_WIDTH_MM) / canvasWidth;
                
                pdf.addImage(partImgData, 'JPEG', 0, 0, A4_WIDTH_MM, partHeightMM);
            }
        }
    }
    ...
    // Ссылка на курс (примерные координаты, подстроить под дизайн)
    // Кнопка "lp.massagestart.ru" - примерно x:130-190mm, y:118-130mm
    pdf.link(130, 116, 58, 14, { url: 'https://lp.massagestart.ru' });
    
    // Ссылка на персональный навигатор
    // Кнопка "Оставить заявку" - примерно x:130-190mm, y:215-227mm  
    pdf.link(130, 212, 58, 14, { url: 'https://forms.gle/sq3ns2Co5CNjoP6h6' });
    
    // Ссылка на email
    pdf.link(70, 252, 70, 10, { url: 'mailto:7days@massagestart.ru' });
...
// ═══════════════════════════════════════════════════════════════════════════
// СОХРАНЕНИЕ В IndexedDB (с автоматическим разбиением страниц)
// ═══════════════════════════════════════════════════════════════════════════
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        if (canvasHeight <= A4_HEIGHT_PX) {
            ...
            for (let j = 0; j < numPages; j++) {
                ...
                const partCtx = partCanvas.getContext('2d');
                partCtx.drawImage(canvas, 0, startY, canvasWidth, partHeight, 0, 0, canvasWidth, partHeight);
